    ...
    componentDidMount() {
        ...
        fetch(`${BASE_API_URL}recorded-course`)
            ...
            .then((data) => {
                if (data.status && data.data) {
                    ...
                    // Group by category (since categories null, fallback)
                    const coursesByCategory = courses.reduce((acc, course) => {
                        const cat = course.categories || "General";
                        if (!acc[cat]) acc[cat] = [];
                        acc[cat].push(course);
                        return acc;
                    }, {});
        ...
        if (user) {
            fetch(`${BASE_API_URL}my-courses/${user.id}`)
                ...
                .then(data => {
                    if (data.status) {
                        this.setState({ enrolledCoursesList: data.data.all || [] });
                    }
                })
                    ...
                    <div className="section_container h-100">
                        ...
                        >
                            {this.state.coursesList?.slice(0, 3).map(course => {
                                const isEnrolled = this.state.enrolledCoursesList.some(ec => ec.id === course.id && ec.enrollment?.status !== "inactive");
                        ...
                        <div className="journey_wrap position-relative">
                            ...
                            <div className="dotted_lines">
                                <div className="position-relative d-flex justify-content-center">
                                    <img src={`${BASE_IMAGE_URL}journey/dotted-lines.png`} className="dotted-line-img" alt="" />
                                </div>
                            </div>
